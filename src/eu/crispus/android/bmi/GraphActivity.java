...
//		requestWindowFeature(Window.FEATURE_NO_TITLE);
    ...
    private void erzeugeGrap() {
        ...
        InputStream is = this.getResources().openRawResource(R.drawable.opengl_buchstaben);
        try {
            characterManager.loadBitmapLetters(BitmapFactory.decodeStream(is));
        } catch (Exception e) {
            Log.e(TAG, "Fehler beim laden der Buchstaben-Texture.", e);
        } finally {
            try {
                is.close();
            } catch (IOException e) {
                Log.e(TAG, "Fehler beim freigeben der Buchstaben-Texture.", e);
            }
        }
        //Zahlen für Charaktermanager
        is = this.getResources().openRawResource(R.drawable.opengl_zahlen);
        try {
            characterManager.loadBitmapDigits(BitmapFactory.decodeStream(is));
        } catch (Exception e) {
            Log.e(TAG, "Fehler beim laden der Buchstaben-Texture.", e);
        } finally {
            try {
...
class GraphRenderer implements GLSurfaceView.Renderer {
     ...
     *
     * @param graph Graph der gezeichnet werden soll.
     */
